 ...
 * </pre>
 * Most {@code java.time} types are serialized as numbers (integers or decimals as appropriate) if
 * the {@link com.fasterxml.jackson.databind.SerializationFeature#WRITE_DATES_AS_TIMESTAMPS} feature
 * is enabled, and otherwise are serialized in standard <a href="http://en.wikipedia.org/wiki/ISO_8601"
 * target="_blank">ISO-8601</a> string representation. ISO-8601 specifies formats for representing
 * offset dates and times, zoned dates and times, local dates and times, periods, durations, zones,
 * and more. All {@code java.time} types have built-in translation to and from ISO-8601 formats.
 * <p>
 * Granularity of timestamps is controlled through the companion features {@link
 * com.fasterxml.jackson.databind.SerializationFeature#WRITE_DATE_TIMESTAMPS_AS_NANOSECONDS} and
 * {@link com.fasterxml.jackson.databind.DeserializationFeature#READ_DATE_TIMESTAMPS_AS_NANOSECONDS}.
 * For serialization, timestamps are written as fractional numbers (decimals), where the number is
 * seconds and the decimal is fractional seconds, if {@code WRITE_DATE_TIMESTAMPS_AS_NANOSECONDS} is
 * enabled (it is by default), with resolution as fine as nanoseconds depending on the underlying
 * JDK implementation. If {@code WRITE_DATE_TIMESTAMPS_AS_NANOSECONDS} is disabled, timestamps are
 * written as a whole number of milliseconds. At deserialization time, decimal numbers are always
 * read as fractional second timestamps with up-to-nanosecond resolution, since the meaning of the
 * decimal is unambiguous. The more ambiguous integer types are read as fractional seconds without a
 * decimal point if {@code READ_DATE_TIMESTAMPS_AS_NANOSECONDS} is enabled (it is by default), and
 * otherwise they are read as milliseconds.
 * <p>
 ...
 * timestamp.</li>
 * <li>{@link ZoneId} and {@link ZoneOffset}, which do not actually store dates and times but are
 * supported with this module nonetheless.</li>
 * <li>{@link LocalDate}, {@link LocalTime}, {@link LocalDateTime}, and {@link OffsetTime}, which
 * cannot portably be converted to timestamps and are instead represented as arrays when
 ...
 * @since 2.6
 */
@SuppressWarnings({"javadoc", "deprecation", "RedundantSuppression"})
